...
## 💡 한계점 및 향후 과제 
**1. 수직 환경 특화 다중 센서 융합 및 SLAM 고도화**
* **문제점:** 매끄러운 철판 표면에서는 라이다의 기하학적 특징점 매칭이 어려워 위치 추정 오차가 발생하며, 수직 등반 시 중력 방향 변화로 인해 IMU와 휠 엔코더 간의 데이터 융합 신뢰도가 저하되는 물리적 한계가 존재했습니다.
* **개선 방안:** EKF(Extended Kalman Filter) 파라미터 최적화를 통해 수직 환경에서의 센서 융합 오차를 보정하고, 특징점이 없는 환경을 극복하기 위해 카메라 비전 기반의 V-SLAM(Visual SLAM) 또는 Depth 카메라를 보조 센서로 도입하는 방안을 고려하고 있습니다.

**2. Edge AI 결함 탐지 파이프라인 고도화**
* **문제점:** 현재 구축된 900장 규모의 데이터셋으로는 실제 선박 하부의 심한 조도 변화나 다양한 형태의 결함을 완벽히 커버하기에 한계가 있습니다.
* **개선 방안:** 선체 부식 및 크랙에 대한 고품질 이미지 데이터셋을 추가 확충하고, 더 높은 정확도와 추론 속도를 보장하는 최신 모델(YOLO11n 등)로 마이그레이션하여 탐지 성능(mAP)을 한 단계 더 끌어올릴 계획입니다. 

**3. 완전 자동화 및 다중 로봇 관제 시스템 확장**
* **문제점:** 현재는 단일 로봇(bot17)이 지정된 목적지로 이동하는 형태의 자율주행을 수행하고 있습니다 [2].
* **개선 방안:** 선체 표면을 빠짐없이 스캔하기 위한 전역 훑기 경로 계획(Coverage Path Planning) 알고리즘을 도입하여 검사 자동화 수준을 높일 것입니다. 또한, 3D 웹 대시보드의 상태 관리 구조(Zustand)를 확장하여 여러 대의 로봇이 동시에 선체를 검사하는 다중 로봇 군집 제어(Fleet Management System) 관제 환경으로 발전시킬 수 있습니다.
