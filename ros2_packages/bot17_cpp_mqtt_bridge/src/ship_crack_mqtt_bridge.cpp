...
public:
    ...
    markerId_(0)
  {
    ...
    declare_parameter("use_relative_pose", true);
    declare_parameter("prefer_odom_pose", true);
    ...
    useRelativePose_ = get_parameter("use_relative_pose").as_bool();
    preferOdomPose_ = get_parameter("prefer_odom_pose").as_bool();
...
private:
  ...
  void onOdom(const nav_msgs::msg::Odometry::SharedPtr msg)
  {
    ...
    if (preferOdomPose_ || !hasLocalizationPose_) {
      const auto &pose = msg->pose.pose;
  ...
  {
    hasPose_ = true;
    const double publishX = useRelativePose_ ? x_ - originX_ : x_;
    const double publishY = useRelativePose_ ? y_ - originY_ : y_;
    const double publishZ = useRelativePose_ ? z_ - originZ_ : z_;
  ...
  void onBattery(const sensor_msgs::msg::BatteryState::SharedPtr msg)
  {
    ...
    if (msg->percentage >= 0.0) {
      percentage = msg->percentage <= 1.0 ? msg->percentage * 100.0 : msg->percentage;
    }
    std::ostringstream json;
  ...
  {
    if (originSet_ || !useRelativePose_) {
      return;
    }
    originX_ = x_;
    originY_ = y_;
    originZ_ = z_;
    originSet_ = true;
    RCLCPP_INFO(get_logger(), "Pose origin set to x=%.3f y=%.3f z=%.3f", originX_, originY_, originZ_);
  }
  ...
  bool hasPose_ = false;
  bool hasLocalizationPose_ = false;
  double x_ = 0.0;
  double y_ = 0.0;
  double z_ = 0.0;
  ...
  bool useRelativePose_ = true;
  bool preferOdomPose_ = true;
